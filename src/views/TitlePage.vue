<template>
  ...
  <el-dialog
    v-model="dispenseProject"
    style="max-width: 800px; padding: 20px"
    align-center
  >
    <template #title>
      <div class="title">标题分配项目</div>
    </template>
    <template #default>
      <el-row class="content" style="width: 400px; margin: 0 auto">
        <el-form label-position="left" style="width: 100%">
          <el-col :span="24">
            <el-form-item label="请分配项目" label-width="120px" required>
              <el-select
                v-model="dispenseProjectList"
                @change="handleDispenseChange"
                multiple
                placeholder="请分配项目"
              >
                <el-option
            ...
            <el-form-item label="分配标题数量" label-width="120px" required>
              <el-input
                v-model="dispenseTitle"
                min="1"
                placeholder="请输入每个项目分配标题数量"
                style="width: 200px"
              ></el-input>
            </el-form-item>
            <el-form-item>
              <el-table :data="duplicateTitles">
                <!-- id列 -->
    ...
    <template #footer>
      <el-button @click="dispenseProject = false">取消</el-button>
      <el-button type="primary" @click="confirmDispenseProject"
        >开始导出</el-button
      >
    </template>
  </el-dialog>
...
// 分配项目标题
const dispenseProject = ref(false);
const dispenseProjectList = ref<string[]>([]);
const dispenseTitle = ref(100);
const duplicateTitles = ref<{ id: string; title: string }[]>([]);
// 批量优化相关状态
...
// 分配项目标题
const handleDispenseProject = async () => {
  dispenseProject.value = true;
};

// 选择分配的项目
const handleDispenseChange = (value: string[]) => {
  dispenseProjectList.value = value;
  // 四舍五入
  dispenseTitle.value = Math.round(productTitles.value.length / value.length);
  if (value.length === 0) {
    dispenseTitle.value = 0;
  }
};

// 确认分配项目标题
const confirmDispenseProject = async () => {
  // 1. 当前标题数是否足够分配

  if (
    productTitles.value.length <
    dispenseTitle.value * dispenseProjectList.value.length
  ) {
    ElMessage.warning(
      `当前标题数不足，请至少再生成 ${
        dispenseTitle.value * dispenseProjectList.value.length -
  ...
  // 2. 检查是否有重复的标题,将重复标题数据保留,回显给用户
  const titleSet = new Set();

  for (const titleItem of productTitles.value) {
    if (titleSet.has(titleItem.title)) {
      duplicateTitles.value.push({
        id: titleItem.id,
        title: titleItem.title,
      });
    } else {
      titleSet.add(titleItem.title);
    }
  }
  if (duplicateTitles.value.length > 0) {
    const result = await ElMessageBox.confirm(
      `检测到 ${duplicateTitles.value.length} 个重复标题，请返回修改或删除再重试操作`,
      "重复标题提醒",
      {
  ...
  const titlesData = [...productTitles.value];
  // 3. 开始生成文档，每个选择的项目目录下生成一个设定标题数的产品标题.xlsx
  dispenseProjectList.value.forEach(async (folderName, index: number) => {
    // 1. 获取path
    const project: { [key: string]: any } = projectList.value.filter(
      (p) => p.folderName === folderName
    )[0];
    // 2. 分配标题
    const titles: any[] = titlesData.splice(0, dispenseTitle.value);
    await exportTitleData(titles, project.folderPath);
  });
};
...
async function exportTitleData(titles: any[], path: string) {
  const project = projectList.value.find(
    (p) => p.folderName === selectedProject.value
  );
  if (!project) return;
